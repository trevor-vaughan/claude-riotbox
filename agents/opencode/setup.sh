...
opencode_setup() {
	...
	#
	# plugin handling: with Context Mode on, riotbox writes a local plugin shim
	# (agents/opencode/context-mode.sh) that loads the copy vendored in the
	# image. opencode loads a local plugin and an npm plugin of the same name
	# separately, so a host config that also lists `context-mode` would put two
	# adapter instances in one session — the ctx_* tools registered twice and
	# every hook fired twice. Drop the npm entry; the shim is the one riotbox
	# can pin. With Context Mode off there is no shim, so the entry is the
	# user's own plugin choice and stays.
	local drop_cm=false
	...
	merged="$(jq -n \
        ...
        | (if $drop_cm and (.plugin | type) == "array"
           then .plugin = [.plugin[]
                           | select(if type == "string"
                                    then (. != "context-mode"
                                          and (startswith("context-mode@") | not))
                                    else true
                                    end)]
           else .
           end)
	...
	# Say what was taken out, and only when something was. Nothing else in the
	# pipeline touches .plugin, so the pre-merge array minus the post-merge one
	# is exactly what the clause above removed — a before/after difference
	# rather than a second copy of the predicate to keep in step. The file named
	# is the generated one, not the host config the user edited: that is the
	# file whose plugin array no longer matches what they wrote.
	if [[ "${drop_cm}" = "true" ]]; then
		local dropped
		dropped="$(jq -rn \
			--argjson j "${json_obj}" \
			--argjson jc "${jsonc_obj}" \
        ...
        (($j * $jc).plugin | as_array) - ($merged.plugin | as_array)
        | join(", ")
        ')"
		if [[ -n "${dropped}" ]]; then
			echo "  [context-mode] WARN: dropped ${dropped} from the plugin array in" >&2
			echo "  [context-mode] ${opencode_jsonc} — riotbox supplies its own pinned copy" >&2
			echo "  [context-mode] as a local plugin, and the npm entry would load a second" >&2
			echo "  [context-mode] adapter instance alongside it." >&2
		fi
	fi
	...
	# headroom-exec.sh relies on this exact shape (`//` banner lines + plain
	# JSON) to split banner from body with line-level grep — keep them apart.
	{
		cat <<'BANNER'
